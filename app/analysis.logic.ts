...
export class AnalysisService {
  private readonly MAX_ASYNC_WAIT_MS = 1000
  private readonly MAX_SYNC_WAIT_MS = 1000

  private async asyncGateForScenario(): Promise<void> {
    const waitTime = Math.random() * this.MAX_ASYNC_WAIT_MS
    await new Promise(resolve => setTimeout(resolve, waitTime))
  }

  private async syncGateForScenario(): Promise<void> {
    const waitTime = Math.random() * this.MAX_SYNC_WAIT_MS
    await new Promise(resolve => setTimeout(resolve, waitTime))
  }
  ...
  async getOptimizationAreas(params: OptimizationRequest = {}): Promise<OptimizationResponse> {
    ...
    try {
      ...
      throw new AnalysisDomainError(
        ...
        { processingTime, maxWaitTime: this.MAX_ASYNC_WAIT_MS }
      )
    }
  }
}
